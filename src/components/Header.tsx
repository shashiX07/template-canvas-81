    ...
    <>
      <motion.header
        ...
        animate={{
          ...
          x: isFloating ? '-50%' : '0%',
          width: isFloating ? 'min(720px, 92%)' : '100%',
        ...
        style={{
          background: isFloating
            ...
            : showBackdrop
              ...
              : 'transparent',
          backdropFilter: showBackdrop || isFloating ? 'blur(24px) saturate(180%)' : 'none',
          WebkitBackdropFilter: showBackdrop || isFloating ? 'blur(24px) saturate(180%)' : 'none',
          borderBottom: isFloating ? 'none' : showBackdrop ? '1px solid hsl(var(--border) / 0.3)' : 'none',
          boxShadow: isFloating
            ? '0 8px 32px hsl(var(--primary) / 0.12), 0 0 0 1px hsl(var(--border) / 0.2)'
            : 'none',
        }}
      >
        <div className={`flex items-center justify-between ${isFloating ? 'px-5 py-2.5' : 'container px-4 h-16'}`}>
          <motion.div
          ...
          {mobileMenuOpen && (
            ...
            >
              {navLinks.map((link, i) => (
                ...
                >
                  <Button variant="ghost" className="w-full justify-start rounded-xl" onClick={() => { navigate(link.path); setMobileMenuOpen(false); }}>
                    {link.label}
                  </Button>
                </motion.div>
              ))}
